...
const resolveAccountsMap = (accounts: AssetsAccount[]): Accounts =>
	accounts.reduce((accountsMap: Accounts, account: AssetsAccount) => {
		return {
			...accountsMap,
...
export class ScenarioPlayer {
	...
	constructor(initialCustomerAccounts: AssetsAccount[]) {
	...
	executeAndSettlePayment(tx: Tx) {
		...
		const accounts = Object.values(this._accounts).map(
			({ paymentInstrument }: AssetsAccount) => paymentInstrument
		)

		if (accounts.includes(tx.debitorBankAccount)) {
			const debitAccount: AssetsAccount = this._accounts[tx.debitorBankAccount]
			const nextBalance = (parseFloat(debitAccount.balance) - parseFloat(tx.amount)).toString()
			...
			const nextBalance = (parseFloat(creditAccount.balance) + parseFloat(tx.amount)).toString()
